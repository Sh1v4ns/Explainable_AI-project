...
class MLPSHAP(nn.Module):
    def __init__(self, num_features, num_classes, index_to_name, dataframe, categorical, numerical, link_func=False, add_bias=True):
        ...
        self.fc1 = nn.Linear(len(categorical) * 32 + len(numerical), 128)
        ...
        self.fc4 = nn.Linear(128, num_features * num_classes)
...
def train_model(index_to_name, train_dataloader,
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer_train, factor=0.5, patience=patience // 3, min_lr=5e-4,
        )
    ...
    for epoch in range(current_epoch, max_num_epochs + 1):
        ...
        for i, data in enumerate(tqdm(val_dataloader)):
            ...
            list_prob_pred.extend(outputs.reshape(-1).tolist())
        ...
        probs = np.array(list_prob_pred)
        labels_np = np.array(val_labels)

        best_thresh = 0.5
        best_f1 = 0
       
       #logic for best threshold values
        ...
        roc = roc_auc_score(val_labels, list_prob_pred)

        final_preds = (probs > best_thresh).astype(int)

        prec = precision_score(labels_np, final_preds, average='macro')
        recall = recall_score(labels_np, final_preds, average='macro')
        f_score = f1_score(labels_np, final_preds, average='macro')
